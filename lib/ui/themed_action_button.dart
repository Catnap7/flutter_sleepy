...
class ThemedActionButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final baseBg = cs.primaryContainer.withValues(alpha: 0.88);
    final baseFg = cs.onPrimaryContainer;

    final pressedBg = Color.alphaBlend(
        cs.primary.withValues(alpha: 0.22), cs.primaryContainer);
    final hoveredBg = Color.alphaBlend(
        cs.primary.withValues(alpha: 0.12), cs.primaryContainer);

    final sideIdle = BorderSide(color: cs.primary.withValues(alpha: 0.28));
    final sideHover = BorderSide(color: cs.primary.withValues(alpha: 0.40));
    final sidePress = BorderSide(color: cs.primary.withValues(alpha: 0.55));
    ...
    final style = ButtonStyle(
      ...
      shadowColor: WidgetStateProperty.all(cs.primary.withValues(alpha: 0.20)),
      minimumSize:
          WidgetStateProperty.all(Size(dense ? 0 : 0, dense ? 44 : 48)),
